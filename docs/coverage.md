...
| Concurrent fresh `task` launches from one parent | None for the ambiguous launch | Denied conservatively | Until the preceding child has appeared in `session.created`, a second task from the same parent is denied because two pending calls could not be assigned safely from `parentID` alone. |
